export const Greenbutton = ({color}:{color:string})=>{
   return(
      <button aria-label="contact-us" className="px-3 md:px-4 py-2 bg-green rounded-2xl">
              <svg
                width="20"
                height="11"
                viewBox="0 0 20 11"
...
export function GradientBorderBox({ title }: { title: string }) {
  return (
    <div className="rounded-full p-[2px] border border-white ">
      <button className="rounded-2xl  text-white px-4 md:px-6 py-1 md:py-2">
    {title}
      </button>
    </div>
  );
